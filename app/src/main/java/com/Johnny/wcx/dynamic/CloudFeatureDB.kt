...
object CloudFeatureDB {
    ...
    private fun loadLocalCache() {
        try {
            val cacheFile = File(KnownPaths.moduleData, CACHE_FILE)
            if (!cacheFile.exists()) {
            ...
            val metaFile = File(KnownPaths.moduleData, CACHE_META)
            if (metaFile.exists()) {
    ...
    private fun saveLocalCache() {
        try {
            ...
            File(KnownPaths.moduleData, CACHE_FILE).writeText(json.toString(2))

            val meta = JSONObject()
            meta.put("lastUpdateTime", lastUpdateTime)
            File(KnownPaths.moduleData, CACHE_META).writeText(meta.toString(2))
    ...
    private fun fetchFromCloud(): Boolean {
        try {
            val url = URL("$CLOUD_URL?wechat_version=${HostInfo.versionName}&module_version=${HostInfo.moduleVersion}")
