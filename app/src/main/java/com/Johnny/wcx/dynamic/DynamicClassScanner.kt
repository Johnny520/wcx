        ...
        return try {
            val classes = dexKit.findClass {
                ...
                feature.annotations.forEach { annotation ->
                    matcher { addAnnotation(annotation) }
                }
    ...
    private fun scanByStringConstant(dexKit: DexKitBridge, feature: ClassFeature): ScanResult? {
        ...
        return try {
            ...
            if (feature.superClass != null) {
                candidates = classes.filter { it.superClass == feature.superClass }
    ...
    private fun scanByFuzzyKeyword(dexKit: DexKitBridge, feature: ClassFeature): ScanResult? {
        ...
        return try {
            ...
            val scored = allClasses.map { cls ->
                ...
                val superName = cls.superClass?.lowercase() ?: ""
                for (kw in keywords) {
        ...
        return try {
            ...
            if (results.isEmpty()) {
                if (feature.nameKeywords.isNotEmpty()) {
                    ...
                    if (best != null) {
                        return DynamicFieldDesc(best.className, best.fieldName, best.type)
                    }
                }
                return null
            }
            ...
            val f = results.first()
            DynamicFieldDesc(f.className, f.fieldName, f.type)
        } catch (e: Exception) {
